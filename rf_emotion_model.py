...
# VOI 계산에 b_6k_8k가 필요해서 merge는 유지
# (단, b_6k_8k는 feat_cols에서 제외 — Ablation에서 중복 피처로 확인됨)
...
# - b3_1k_2k 높을수록: 1-2kHz 에너지 높음 = 스트레스 특성 (p=0.046 검증)
...
# - b_6k_8k 높을수록: 고주파 에너지 높음 = happy 특성 (p=0.003 검증)
# - b3_1k_2k 낮을수록: stressed 특성이 아님
# → 높을수록 happy, 낮을수록 stressed/tired
# Note: b_6k_8k는 VOI 계산에만 사용, feat_cols에는 포함 안 함 (중복 피처)
main['voi'] = (
...
feat_cols = [
    ...
    'b3_1k_2k',             # 중음 (1-2kHz) — stressed 특성 (p=0.046)
    'b4_2k_4k',             # 중고음 (2-4kHz)
    'b5_4k_8k',             # 고음 (4-8kHz)
    ...
    # 새로 설계한 해석 가능한 음향 지표
    'vts',                  # Vocal Tension Score (긴장도)
    'voi',                  # Vocal Openness Index (밝음/열림)
    # b_6k_8k 제외: VOI와 중복 정보 — Ablation 검증 결과
...
# n_estimators=200: 데이터 114개 기준 최적값
...
print("+ 하이퍼파라미터 튜닝:        F1=0.806  (+0.011)")
print("+ b_6k_8k 제거 (중복 제거):   F1=0.816  (+0.010)  ← 최종")
